...
from setting.utils import to_sparse_tensor
from torch_geometric.nn import GATConv,GATv2Conv
from setting.nn import  gcn
...
def sparse_or_dense_dropout(x, p=0.5, training=True):
    if isinstance(x, (torch.sparse.FloatTensor, torch.cuda.sparse.FloatTensor)):
        ...
        return torch.sparse_coo_tensor(x.indices(), new_values, x.size(), dtype=new_values.dtype, device=new_values.device)
...
class GAT(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dims, output_dim, dropout=0.5, batch_norm=False):
        ...
        self.layers = nn.ModuleList([GATConv(input_dim, layer_dims[0],heads = 6,add_self_loops=True,concat = False)])
        self.layers.append(GATConv(hidden_dims[0] ,output_dim , heads = 6,add_self_loops=True, concat = False ))#heads was 6
